...
def generate_slide_image(prompt: str) -> str | None:
    """Generates an educational illustration using the Gemini 3.1 Flash Lite Image model or Imagen fallback."""
    generation_config = {
        'temperature': 1,
    ...
    try:
        if hasattr(client, 'interactions'):
            interaction = client.interactions.create(
                ...
                input=f"Minimalist educational diagram, scientific illustration of: {prompt}, clean dark background, vector style",
    ...
    try:
        response = client.models.generate_images(
            ...
            prompt=f"Minimalist educational diagram, scientific illustration of: {prompt}, clean dark background, vector style",
...
def generate_lesson_notes(concept: dict, prereq_labels: list[str]) -> dict:
    ...
    prompt = f"""
    You are an expert curriculum designer creating a 4-slide study deck and cheat sheet for a student.
    ...
    {{
      ...
      "slides": [
        {{
          ...
          "image_prompt": "Clean educational vector diagram illustration of key terms for {concept['label']}"
        }},
        {{
          "slide_number": 2,
          ...
          "image_prompt": "Mathematical or conceptual diagram showing core formula for {concept['label']}"
        }},
        {{
          "slide_number": 3,
          ...
          "worked_code": "Step 1: ... \\nStep 2: ... \\nAnswer: ...",
          "image_prompt": "Step-by-step visual flowchart diagram for {concept['label']}"
        }},
        {{
          "slide_number": 4,
          ...
          "image_prompt": "Summary cheat sheet visual icons for {concept['label']}"
        }}
      ]
    }}
    """
    ...
    for slide in notes_data.get("slides", []):
        if slide.get("image_prompt"):
            slide["image_url"] = generate_slide_image(slide["image_prompt"])

    return notes_data

def generate_level_notes(level_name: str, concepts: list[dict]) -> dict:
    """Uses Gemini to generate level-by-level study slide deck covering all concepts in a tier."""
    concept_names = [c['label'] for c in concepts]
    concept_descriptions = [f"{c['label']}: {c.get('description', '')}" for c in concepts]
    
    prompt = f"""
    You are an expert curriculum designer creating a Level Study Deck for {level_name}.
    ...
    Format the notes into EXACTLY 5 structured slides:
    ...
    {{
      ...
      "slides": [
        {{
          ...
          "image_prompt": "Educational blueprint roadmap diagram for {level_name}"
        }},
        {{
          "slide_number": 2,
          ...
          "image_prompt": "Glossary vector diagram for {level_name}"
        }},
        {{
          "slide_number": 3,
          ...
          "image_prompt": "Mathematical structure diagram for {level_name}"
        }},
        {{
          "slide_number": 4,
          ...
          "image_prompt": "Step-by-step problem solving flowchart for {level_name}"
        }},
        {{
          "slide_number": 5,
          ...
          "image_prompt": "Master cheat sheet summary diagram for {level_name}"
        }}
      ]
    }}
    """
