          ...
          <div key={idx} className="pdf-slide-page">
            ...
            <div className="w-full flex justify-between items-center border-b border-zinc-800 pb-3 mb-4">
              <div className="flex items-center gap-2">
                <span className="px-3 py-1 rounded-full bg-[#da6b38] text-white text-xs font-mono font-bold">
              ...
              <h2 className="text-base font-extrabold text-white font-heading">{slide.title}</h2>
            </div>

            {/* 16:9 Presentation Image & Content Layout */}
            <div className="w-full flex-1 grid grid-cols-12 gap-6 items-center">
              {slide.image_url ? (
                <div className="col-span-7 h-full max-h-[70vh] rounded-2xl overflow-hidden border border-zinc-800 bg-black flex items-center justify-center">
                  <img src={slide.image_url} alt="" className="w-full h-full object-contain" />
                </div>
              ) : (
                <div className="col-span-7 h-full max-h-[70vh] rounded-2xl border border-zinc-800 bg-[#13151a] p-6 flex flex-col justify-between">
                  <div className="text-xs font-mono text-[#da6b38] font-bold">Slide Blueprint</div>
                  <h3 className="text-lg font-bold text-white font-heading">{slide.title}</h3>
                  <div className="text-xs font-mono text-zinc-500">MasteryMap AI Presentation Deck</div>
                </div>
              )}
              ...
              <div className="col-span-5 space-y-4">
                {slide.content_bullets && (
                  <ul className="space-y-3">
                    {slide.content_bullets.map((b, i) => (
                      <li key={i} className="text-sm text-zinc-200 flex items-start gap-2.5 font-sans leading-relaxed">
                        <span className="w-2 h-2 rounded-full bg-[#da6b38] mt-1.5 shrink-0"></span>
                        <span>{b}</span>
                      </li>
                    ))}
                  </ul>
                ...
                {slide.key_term && (
                  <div className="p-4 rounded-xl bg-zinc-900 border border-zinc-800 text-xs text-zinc-300 font-mono space-y-1">
                    <div className="text-[10px] text-[#da6b38] uppercase font-bold">Key Concept</div>
                    <div>{slide.key_term}</div>
                  </div>
                )}

                {slide.formula && (
                  <div className="p-4 rounded-xl bg-zinc-900 border border-zinc-800 text-xs text-cyan-300 font-mono space-y-1">
                    <div className="text-[10px] text-zinc-400 uppercase font-bold">Core Equation</div>
                    <div className="text-sm font-bold text-cyan-200">{slide.formula}</div>
                  </div>
                )}
              </div>
            </div>
            ...
            {/* Footer */}
            <div className="w-full border-t border-zinc-800 pt-3 mt-4 flex justify-between items-center text-[10px] font-mono text-zinc-500">
              <span>MasteryMap Adaptive Presentation Deck</span>
